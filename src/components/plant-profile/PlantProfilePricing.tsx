...
const PlantProfilePricing: React.FC<PlantProfilePricingProps> = ({
  ...
  const handleAddPriceTier = () => {
    ...
    const defaultTier: PriceTier = {
      quantity: newPlant.priceTiers?.length > 0 ? 
        (newPlant.priceTiers[newPlant.priceTiers.length - 1].quantity + 500) : 
        500,
      size: "12",
      type: "1LP",
      price: newPlant.priceTiers?.length > 0 ? 
        (newPlant.priceTiers[newPlant.priceTiers.length - 1].price * 0.9) : 
        15
    };
    
    newPlant.priceTiers = [...(newPlant.priceTiers || []), defaultTier];
    setPlant(newPlant);
  };
  ...
  React.useEffect(() => {
    ...
    if (!newPlant.priceTiers && newPlant.prices) {
      ...
      needsUpdate = true;
    } else if (!newPlant.priceTiers && !newPlant.prices) {
      newPlant.priceTiers = [{
        quantity: 500,
        size: "12",
        type: "1LP",
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {plant.priceTiers?.map((tier, index) => (
            <div key={index} className="mb-6 pb-6 border-b border-border last:border-0 last:pb-0">
              ...
              <div className="flex justify-end">
                <Button 
                  ...
                  disabled={disabled || (plant.priceTiers?.length || 0) <= 1}
                >
